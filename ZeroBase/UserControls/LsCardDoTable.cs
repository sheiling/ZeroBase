...
namespace ZeroBase
{
    ...
    public partial class LsCardDoTable : UserControl
    {
                        ...
                        if (XDevice.Instance.CardMap[cardid].Name == cardname)
                        {
                            ...
                            foreach (KeyValuePair<int, XDo> kvp in XDevice.Instance.DoMap)
                            {
                                if (kvp.Value.CardName == cardname)
                                {
                                    ...
                                    dr.Cells[3].Value = kvp.Value.SetId;
                                    dr.Cells[4].Value = kvp.Value.ActId+1;

                                    dr.Cells[5].Value = kvp.Value.Name;
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 1)
            {
                int doId = int.Parse(dataGridView1.Rows[e.RowIndex].Cells[3].Value.ToString());
            ...
            if (EnableTimer)
            {
                try
                {
                    foreach (DataGridViewRow dr in dataGridView1.Rows)
                    {
                            ...
                            int doId = int.Parse(dr.Cells[3].Value.ToString()) ;
